...
class pinyinTransformerEncoderLayer(nn.Module):
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x,encoder, encoder_padding_mask, layer_num, attn_mask: Optional[Tensor] = None, src_dpd_matrix: Optional[Tensor] = None):
        """
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
            encoder_padding_mask (ByteTensor): binary ByteTensor of shape
