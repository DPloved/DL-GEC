...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers, already_numberized=False,
        ...
        merge_result(
            Binarizer.binarize(
                ...
                already_numberized=already_numberized, append_eos=True
            )
        )
        if num_workers > 1:
            pool.join()
    ...
    def make_tag_dataset(input_prefix, output_prefix, already_numberized=True,
                            append_eos=False):
        ...
        input_file=input_file.replace('char.','')
        offsets = Binarizer.find_offsets(input_file, 1)

        ds = indexed_dataset.make_builder(
            args.destdir+'/'+output_prefix+".src-tgt.tag.bin",
            impl=args.dataset_impl,
            vocab_size=4,
        )
        ...
        Binarizer.binarize_tag(
            ...
            already_numberized=already_numberized, append_eos=False
        )

        ds.finalize(args.destdir+'/'+output_prefix+".src-tgt.tag.idx")

    def make_pinyin_dataset(input_prefix, output_prefix, already_numberized=True,
                            append_eos=False):


        input_file = "{}{}".format(
            input_prefix, ("." + 'pinyin.tag')
        )
        input_file=input_file.replace('char.','')
        offsets = Binarizer.find_offsets(input_file, 1)

        ds = indexed_dataset.make_builder(
            args.destdir+'/'+output_prefix+".src-tgt.pinyin.bin",
            impl=args.dataset_impl,
            vocab_size=437,
        )
        ...
        ds.finalize(args.destdir+'/'+output_prefix+".src-tgt.pinyin.idx")
    ...
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1):
        if args.dataset_impl == "raw":
            ...
            if lang == "src_nt_bart":
                make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers, already_numberized=True,
                                    append_eos=True)
            else:
                make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers,append_eos=True)
    ...
    def make_all_tag():
        if args.trainpref:
            make_tag_dataset(args.trainpref, "train")
        if args.validpref:
            outprefix = "valid"
            make_tag_dataset(
                args.validpref, outprefix
            )

    def make_all_pinyin():
        if args.trainpref:
            make_pinyin_dataset(args.trainpref, "train")
        if args.validpref:
            outprefix = "valid"
            make_pinyin_dataset(
                args.validpref, outprefix
...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, already_numberized=False, append_eos=True):
    ...
    res = Binarizer.binarize(
        filename, vocab, consumer, append_eos=True, offset=offset, end=end, already_numberized=already_numberized
    )
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
