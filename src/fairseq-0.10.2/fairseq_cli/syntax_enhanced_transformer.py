...
class SyntaxEnhancedTransformerModel(FairseqEncoderDecoderModel):
    ...
    def __init__(self, args, encoder, decoder,head,crf):
    ...
    def build_model(cls, args, task):
        ...
        return cls(args=args, encoder=encoder, decoder=decoder, head=head, crf=crf)
    ...
    def forward(
        ...
        if tag is not None:
            ...
            encoder_dropout = self.dropout(encoder_out.encoder_out)
            lstm_out, hidden = self.lstm(encoder_dropout)
            tag_lost = self.head(lstm_out)
          #  print(tag_lost.size())
          #  print(encoder_padding_mask.size())
            tag_lose = self.crf(emissions=tag_lost.transpose(1,0), tags=tag, mask=encoder_padding_mask) * -1
...
class CRF(nn.Module):
    ...
    def forward(self, emissions: torch.Tensor,
        ...
        label_weights = torch.tensor([1, 0.05, 0.05, 0.05, 0.05], dtype=torch.float16)
        label_weights = label_weights.to(self.start_transitions.device)
        # shape: (batch_size,)
        numerator = self._compute_score(emissions, tags, mask,label_weights)
        # shape: (batch_size,)
        denominator = self._compute_normalizer(emissions, mask,label_weights)
        # shape: (batch_size,)
    ...
    def _compute_score(self, emissions: torch.Tensor,
                       tags: torch.LongTensor,
                       mask: torch.ByteTensor,label_weights: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(1, seq_length):
            ...
            # Emission score for next tag, only added if next timestep is valid (mask == 1)* label_weights[tags[i]]
            # shape: (batch_size,)* label_weights[tags[i]]
    ...
    def _compute_normalizer(self, emissions: torch.Tensor,
                            mask: torch.ByteTensor, tag_weights: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(1, seq_length):
            ...
            # Add tag_weights to emissions for each current tag
            weighted_emissions = broadcast_emissions + tag_weights.unsqueeze(0).unsqueeze(0)

            next_score = broadcast_score + self.transitions + weighted_emissions
