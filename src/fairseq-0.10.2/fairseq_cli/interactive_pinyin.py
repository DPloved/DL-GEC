...
Batch = namedtuple("Batch", "ids src_tokens src_lengths pinyin constraints src_nt src_nt_lengths src_outcoming_arc_mask src_incoming_arc_mask src_dpd_matrix src_probs_matrix")
...
def make_batches(lines, pinyin,args, task, max_positions, encode_fn):
    ...
    if args.src_nt_file:
        if indexed_dataset.dataset_exists(args.src_nt_file, impl="mmap"):
            ...
            raise FileNotFoundError
    pinyin_list=[]
    for string in pinyin:
        numbers = [int(num) for num in string.split() if num.isdigit()]
        pinyin_list.append(torch.tensor(numbers))
    itr = task.get_batch_iterator(
        dataset=task.build_dataset_for_pinyin_inference(
            tokens, lengths,pinyin=pinyin_list,constraints=constraints_tensor, src_nt=src_nt, src_nt_sizes=src_nt_sizes, src_conll_dataset=src_conll_dataset, src_dpd_dataset=src_dpd_dataset, src_probs_dataset=src_probs_dataset, syntax_type=args.syntax_type
        ),
    ...
    for batch in itr:
        ...
        pinyin_tokens = batch["net_input"]['pinyin']
...
def main(args):
    ...
    start_id = 0
    #print(args.input)
    for inputs,pinyin in zip(buffered_read(args.input[0],args.buffer_size),buffered_read(args.pinyin,args.buffer_size)):


    #for inputs,pinyin in zip(buffered_read(args.input,args.buffer_size),buffered_read('/home/shared/data/zh_data/HSK/nlpcc18.pinyin',args.buffer_size)):
        results = []
        for batch in tqdm(make_batches(inputs,pinyin, args, task, max_positions, encode_fn)):
            bsz = batch.src_tokens.size(0)
            src_tokens = batch.src_tokens
            src_lengths = batch.src_lengths
            pinyin_tokens = batch.pinyin
            ...
            if use_cuda:
                ...
                if pinyin_tokens is not None:
                    pinyin_tokens =pinyin_tokens.cuda()
...
def cli_main():
    ...
    parser.add_argument('--pinyin', type=str, help='Paths to the input files')
