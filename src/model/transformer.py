...
class SyntaxEnhancedTransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        return cls(args=args, encoder=encoder, decoder=decoder, crf=crf, head=head)
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            src_tokens, src_lengths=src_lengths, pinyin=pinyin, src_outcoming_arc_mask=src_outcoming_arc_mask,
            src_incoming_arc_mask=src_incoming_arc_mask, src_dpd_matrix=src_dpd_matrix,
            src_probs_matrix=src_probs_matrix, source_tokens_nt=source_tokens_nt,
            source_tokens_nt_lengths=source_tokens_nt_lengths, return_all_hiddens=return_all_hiddens
        )
        ...
        if tag is not None:
            ...
            #encoder_padding_mask[:, 0] = True
            encoder_layer = self.layer_focus(self.layernorm(encoder_out[3][11]))
            lstm, hidden = self.lstm(encoder_layer)
            #  lstm = self.tanh(lstm)
            lstm = self.dropout(lstm)
            #lstm = self.layernorm(lstm)
            # print(src_tokens)
            #   print('src_token',src_tokens)
            #  print('encoder_pading',encoder_padding_mask)
            #  encoder_dropout = self.dropout(encoder_out[3][11])
            # loss= self.crf.neg_log_likelihood(encoder_out.encoder_out,tag)
            #  lstm = self.dropout(lstm)
            # print(tag)
            # lstm = self.relu(lstm)
            # lstm = self.dropout(lstm)
            # print(lstm.size())
            # gru ,hidden = self.gru(encoder_out[3][11])
            # cat_tensor=torch.cat((gru*self.weight[0],lstm*self.weight[1]),dim=-1)
            #extend_encoder =  torch.cat([encoder_layer,encoder_layer], dim=-1)
            # forget_gate_output = self.sigmoid(self.forget_gate(lstm_out))
            #lstm = lstm +extend_encoder
            #  encoder_dropout = self.relu(encoder_dropout)
            # encoder_dropout = self.sigmoid(encoder_dropout)
            # print(hidden[0].size())
            # print(hidden[1].size())
            tag_lost = self.head(lstm)
            #  print(tag_lost.size())
            #  print(encoder_padding_mask.size())
            # tag_lost = self.sigmoid(tag_lost)
            # self.merit(tag_lost,tag,encoder_padding_mask.float())
            # loss_d = sequence_cross_entropy_with_logits(tag_lost, tag, encoder_padding_mask)
            tag_lose = -1 * self.crf(emissions=tag_lost.transpose(1, 0), tags=tag, mask=encoder_padding_mask)

            decoder_out = decoder_out + (tag_lose,)
        return decoder_out
...
class SyntaxEnhancedTransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        sentence_encoder_out = self.sentence_encoder(
            src_tokens, src_lengths=src_lengths, pinyin=pinyin, return_all_hiddens=return_all_hiddens,
            src_dpd_matrix=None
        )  # B * L * D
...
class SyntaxEnhancedSentenceTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.pinyin_embed = Embedding(num_embeddings=421, embedding_dim=1024,padding_idx=420)
        self.embed_tokens = embed_tokens
        #self.pyatt = pinyin_MultiHeadAttention(1024, 8)
        self.embed_scale = 1.0 if args.no_scale_embedding else math.sqrt(
            embed_dim)  # if full mask then embed_scale must be 1

       # self.pin_embed_positions = (
       #     PositionalEmbedding(
       ...
       #     )
        ...
        self.concat_layer = Linear(1024,1024,bias=True)
        ...
        self.pin_lstm = nn.LSTM(1024, 512, num_layers=1,bidirectional=True, dropout=0.1,batch_first=True)
        self.pinyin_layers = nn.ModuleList([])
        self.pinyin_layers.extend(
            [self.build_pinyin_layer(args) for i in range(8)]
        )
    ...
    def build_pinyin_layer(self, args):
        return pinyinTransformerEncoderLayer(args)
    ...
    def forward_embedding(
            self, src_tokens, token_embedding: Optional[torch.Tensor] = None, pinyin=None
    ):
        ...
        if pinyin is not None:
            pin_embed = self.pinyin_embed(pinyin)
           # pin_embed = self.SRC_dropout(pin_embed, 0.3)
           # pin_embed = pin_embed+self.pin_embed_positions(pinyin)
            # x = x+pin_embed
        else:
            pin_embed = None
    ...
    def forward(
        ...
        """
        x, encoder_embedding, pin_embed = self.forward_embedding(src_tokens, token_embeddings, pinyin)
       ...
       # resudal=x
        ...
        for idx, layer in enumerate(self.layers):
           ...
           # print(pin_embed.size())
           # print(x.size())

        if self.layer_norm is not None:
            x = self.layer_norm(x)
        if pin_embed is not None:
            pin_embed,hiden =self.pin_lstm(pin_embed)
            pin_embed =pin_embed.transpose(1,0)
            #pin_embed = self.SRC_dropout(pin_embed, 0.3)
            for idx, layer in enumerate(self.pinyin_layers):
                pin_embed = layer(pin_embed,x, encoder_padding_mask, layer_num=idx, src_dpd_matrix=src_dpd_matrix)
          #  pin_embed = self.layer_norm(pin_embed)
            pin_embed = self.concat_layer(pin_embed)
            pin_embed = self.active(pin_embed)
            pin_sentence_embed = pin_embed * x
            x = pin_sentence_embed+x
          #  x = self.SRC_dropout(x, 0.3)
        return EncoderOut(
...
class BARTClassificationHead(nn.Module):
    ...
    def __init__(
      ...
      #  nn.init.uniform_(self.dense.weight, -0.01, 0.01)
        #  nn.init.kaiming_uniform_(self.dense.bias)
...
#        if do_spectral_norm:
 ...
 #         self.dense.reset_parameters()
  #        self.out_proj.reset_parameters()
    #   nn.init.uniform_(self.dense, -0.1, 0.1)
    #     nn.init.uniform_(self.out_proj, -0.1, 0.1)
