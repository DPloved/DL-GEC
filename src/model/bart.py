...
from .syntax_enhanced_transformer import SyntaxEnhancedTransformerModel, syntax_enhanced_transformer, syntax_enhanced_transformer_big, BARTClassificationHead
from fairseq.modules.transformer_sentence_encoder import init_bert_params
from .hub_interface import BARTHubInterface
#from .mertic import Seq2LabelsMetric
...
class SyntaxEnhancedBARTModel(SyntaxEnhancedTransformerModel):
    ...
    def __init__(self, args, encoder, decoder,crf,head):
        super().__init__(args, encoder, decoder)

        # We follow BERT's random weight initializatioelf.model._init_weights(self.classification_head.out_proj)
        self.apply(init_bert_params)
        self.head=head
        self.crf=crf
        #self.merit=Seq2LabelsMetric()
        self.lstm = nn.LSTM(1024, 1024, num_layers=1,bidirectional=True, dropout=0.2,batch_first=True)
        self.dropout = nn.Dropout(0.2)
        self.layer_focus = nn.Linear(1024,1024,bias=True)
    ...
    def load_bart_state_dict_from_transformers(self, model, strict=True, args=None):
        ...
        #shared_weight = new_state_dict["shared.weight"]
