...
class Seq2LabelsMetric(Metric):
    ...
    def __repr__(self):
        s = f" labels_accuracy: {self.Tags_Accuracy}.\n"
        return s

    @overrides
    def __call__(self,  logits_d, d_tags, mask=None, crf_result=None):
        logits_d, d_tags, mask = self.detach_tensors( logits_d, d_tags ,mask)
     #   num_labels = logits_labels.size(-1)
        num_tags = logits_d.size(-1)
#
 #bels_Accuracy       logits_labels = logits_labels.view((-1, num_labels))
 ...
 #       _correct_labels_ueq_keep_count = correct_labels_ueq_keep.sum()
  ...
  #      self.labels_correct += _correct_labels_count
    ...
    def get_metric(self, reset: bool = False, model_object=None):
        ret = {"labels_accuracy": self.Tags_Accuracy}
        if reset:
            self.reset()
        return ret
